...
// metalCtx wraps the opaque C MetalCtx pointer.
type metalCtx struct {
	ptr *C.MetalCtx
}

func metalInitGo(sdlWin unsafe.Pointer, dpiScale float32) (*metalCtx, error) {
	ctx := C.metalInit(sdlWin, C.float(dpiScale))
	if ctx == nil {
		return nil, fmt.Errorf("gpu: metalInit failed")
	}
	return &metalCtx{ptr: ctx}, nil
}

func (m *metalCtx) newTexture(w, h int) uint64 {
	return uint64(C.metalNewTex(m.ptr, C.int(w), C.int(h)))
}

func (m *metalCtx) updateTexture(id uint64, data []byte, w, h int) {
	C.metalUpdateTex(m.ptr, C.uint64_t(id),
		unsafe.Pointer(&data[0]), C.int(w), C.int(h))
}

func (m *metalCtx) deleteTexture(id uint64) {
	C.metalDeleteTex(m.ptr, C.uint64_t(id))
}

func (m *metalCtx) render(verts []Vertex, cmds []drawCmd,
...
func (m *metalCtx) drawableSize() (int, int) {
	var w, h C.int
